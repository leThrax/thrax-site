...
This started as the default `npm create vite` react-ts scaffold, grew into a single-page filterable showcase of the site owner's tech stack, and is now a multi-page personal portfolio built around that same showcase — see "Site structure" for the page/routing overview and "Tech-stack showcase" for the original page's architecture, still intact as one of the pages.
...
| `/stack` | `TechStackPage` | The original tech-stack showcase — see "Tech-stack showcase" below |
...
All five public-page routes plus `/`, `/stack`, `/cv`, `/projects`, `/blog`, `/blog/:slug`, and `/hardware` are nested under a shared `<Route element={<SiteLayout/>}>` in `main.tsx`; `/admin` is registered as its own sibling top-level route, not wrapped in `SiteLayout` (no public nav or boot sequence there — it stays a distinct control-panel surface).

`src/layouts/SiteLayout.tsx` owns what used to live directly in `App.tsx` back when the tech-stack page was the site's only public route: the `MotionConfig`/`AnimatePresence` boot-sequence gate (`BootSequence`, still session-gated, still bypassed under `prefers-reduced-motion`) and the outer page-load `containerVariants`/`sectionVariants` fade-in wrapper (now shared, in `src/lib/motionVariants.ts`, rather than local to one page — Framer Motion propagates variants through React context, not DOM parentage, so a page's own `motion.div` sections still stagger correctly even though they render inside `SiteLayout`'s `<Outlet/>` rather than as its direct children). It also renders `src/components/SiteNav.tsx`, a persistent shell-flag-styled nav bar (`--stack`, `--cv`, `--projects`, `--blog`, `--hardware`, plus a `~` home link) using `NavLink` for active-route highlighting — shared across every public page via `src/data/navLinks.ts`, which both `SiteNav` and `HomePage`'s quick-link cards read from.

`src/components/WhoamiHeader.tsx` is `HomePage`'s hero: the same bordered "terminal window" chrome as `FetchHeader` (window-dot header bar, `visitor@thrax-site` line), but showing static site/owner info instead of filter state — kept as a separate component rather than overloading `FetchHeader`, which stays specific to the tech-stack page's filter flags.

**CV page**: content lives in `src/data/cv.ts` (a typed `CvData` object — experience/education/skills/links), hand-edited like any other source file. Currently placeholder content (`TODO` strings) — replace with real details. Not admin-editable; CV content changes rarely enough that a Supabase table/admin form would be overhead without benefit.
...
**Blog**: posts are Supabase-backed (`posts` table — see "Supabase layer") with a markdown `body`, authored through a new admin form (`PostForm`/`PostList`, a second managed collection in `AdminPage` alongside items, switched via a simple Items/Posts tab). `published_at` being null means "draft" — RLS lets the admin see/edit drafts but the public `select` policy only returns rows where `published_at is not null`; `BlogIndexPage` additionally filters client-side to published posts as a defense-in-depth measure since `usePosts()` is shared with the admin's draft-inclusive list. `BlogPostPage` fetches a single post by slug (`fetchPostBySlug`, via `useParams().slug`) and renders `body` with `react-markdown` + `remark-gfm` inside a `font-sans` (Inter) container — same mono-headings/humanist-body-prose convention `ItemDetail` already uses for description text — styled via scoped Tailwind arbitrary-value selectors (`[&_h2]:...`, `[&_pre]:...`) rather than the `@tailwindcss/typography` plugin, to avoid adding a dependency for a single page.
...
- **RLS**: public `select` on `items`/`tags`/`categories`/`pinned_repos`/`devices` (anon key, read-only, unconditional); public `select` on `posts` gated to published rows (see above); `insert`/`update`/`delete` on `items`, `posts`, and `devices`, and `insert`/`delete` on `tags`, `categories`, and `pinned_repos`, restricted to `auth.uid() = '<admin-user-id>'`. Same read-public/write-admin-only pattern on the `icons`, `post-media`, and `device-models` buckets.
...
**Free-text search** (`SearchBox.tsx`, `matchesSearch`/`searchItems` in `filtering.ts`): a separate, simpler mechanism from tag faceting — plain case-insensitive substring match on `item.name` only (not description/tags), deliberately not URL-synced (`TechStackPage.tsx` holds `searchQuery` in plain `useState`) since shareable search links weren't asked for, unlike the tag filters. Composed in `TechStackPage.tsx` as `filterItems(searchItems(items, searchQuery), selected)` — search narrows first, tag-faceting applies on top, so the two combine with AND semantics. Styled as an inline `❯ ` prompt-prefixed underline input rather than a boxed search field, matching `FilterBar`'s prompt-symbol convention instead of introducing a different visual language for search vs. filter.
...
- `AdminPage.tsx` has an `activeTab` (`'items' | 'posts' | 'projects' | 'hardware'`) state switching between the items UI (unchanged), the posts UI (`PostList`/`PostForm`, added for the blog), the pinned-repos UI (`PinnedRepoList`, added for contributor repos), and the devices UI (`DeviceList`/`DeviceForm`, added for the hardware page) — the four collections don't interact, so this is a plain tab toggle rather than shared state.
...
```
src/
  types/item.ts, filter.ts, category.ts, post.ts, github.ts, device.ts   # github.ts's GithubRepo has an optional `contributor` flag, see "Pinned (contributed) repos" above
  data/
  ...
  hooks/
    ...
    useTypewriter.ts                # delete-then-type animation, used by FetchHeader's / WhoamiHeader's Shell: line
  layouts/
    SiteLayout.tsx                 # boot-sequence gate + SiteNav + <Outlet/>, wraps every public page — see "Site structure" above
  components/
    ...
    WhoamiHeader.tsx                # HomePage's equivalent hero panel, see "Site structure" above
    SiteNav.tsx                     # persistent shell-flag-styled nav bar, see "Site structure" above
    ...
    Icon.tsx
    hardware/
      DeviceScene.tsx                # <Canvas> + OrbitControls + useGLTF-loaded, bbox-normalized device.modelUrl — see "Hardware page" above
  ...
  pages/
    HomePage.tsx, TechStackPage.tsx, CvPage.tsx, ProjectsPage.tsx, BlogIndexPage.tsx, BlogPostPage.tsx, HardwarePage.tsx
  admin/
    AdminPage.tsx, LoginForm.tsx, ItemForm.tsx, ItemList.tsx, TagPicker.tsx, CategoryPicker.tsx, PostForm.tsx, PostList.tsx, PinnedRepoList.tsx, DeviceForm.tsx, DeviceList.tsx
...
- **`react-router-dom`** handles all client-side routing (see "Site structure" above); **`react-markdown`** + **`remark-gfm`** + **`rehype-raw`** render blog post bodies (`BlogPostPage.tsx`, the last one specifically to allow the raw `<video>` tag through — see "Blog images/video" above); **`three`** + **`@react-three/fiber`** + **`@react-three/drei`** render the `/hardware` page's `.glb` models (see "Hardware page" above) — installed with `--legacy-peer-deps` since `@react-three/fiber`'s peer range (`react >=19 <19.3`) hadn't caught up to the installed React 19.3 patch release at the time; no runtime incompatibility observed. These are the only dependency additions since the site was a single page.
